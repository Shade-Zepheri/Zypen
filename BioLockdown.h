...
#define BIOLOCKDOWN_AUTHENTICATE_APP(ident, success, failure_) \
	...
	{ \
		[[objc_getClass("BioLockdownController") sharedController] authenticateForIdentifier:ident actionDescription:@"Zypen requires authentication" completion:success failure:failure_]; \
	} \
	else \
		success()
