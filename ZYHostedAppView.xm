...
- (void)_preloadOrAttemptToUpdateReachabilityCounterpart {
    if (app) {
        if ([app mainScene]) {
            isPreloading = NO;
            if (((SBReachabilityManager*)[%c(SBReachabilityManager) sharedInstance]).reachabilityModeActive && [[%c(SBMainWorkspace) _instanceIfExists] respondsToSelector:@selector(ZY_updateViewSizes)]) {
                [[%c(SBMainWorkspace) _instanceIfExists] performSelector:@selector(ZY_updateViewSizes) withObject:nil afterDelay:0.5]; // App is launched using ReachApp - animations commence. We have to wait for those animations to finish or this won't work.
            }
