...
- (UIImage*)rotateImageToMatchOrientation:(UIImage*)oldImage
{
	...
	//CGSize rotatedSize = CGSizeApplyAffineTransform(oldImage.size, CGAffineTransformMakeRotation(DEGREES_TO_ZYDIANS(degrees)));
...
-(UIImage*) renderPreviewForDesktop:(ZYDesktopWindow*)desktop
{
	@autoreleasepool {
		...
		//	CGContextRotateCTM(c, DEGREES_TO_ZYDIANS(90));
		UIImage *image = UIGraphicsGetImageFromCurrentImageContext();
		UIGraphicsEndImageContext();
...
-(UIImage*) wallpaperImage:(BOOL)blurred
{
    ...
    [[%c(SBWallpaperController) sharedInstance] beginRequiringWithReason:@"ZYWallpaperSnapshot"];
	...
	UIGraphicsEndImageContext();
	MSHookIvar<UIWindow*>([%c(SBWallpaperController) sharedInstance], "_wallpaperWindow").layer.contents = nil;
	[[%c(SBWallpaperController) sharedInstance] endRequiringWithReason:@"ZYWallpaperSnapshot"];
