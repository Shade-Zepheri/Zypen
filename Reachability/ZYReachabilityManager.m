...
- (void)launchTopAppWithIdentifier:(NSString*)identifier {
	...
	[[objc_getClass("SBMainWorkspace") sharedInstance] ZY_launchTopAppWithIdentifier:identifier];
}

- (void)launchWidget:(ZYWidget*)widget {
	//[[objc_getClass("SBWorkspace") sharedInstance] ZY_closeCurrentView];
	[[objc_getClass("SBMainWorkspace") sharedInstance] ZY_setView:[widget view] preferredHeight:[widget preferredHeight]];
}

- (void)showWidgetSelector {
	//[[objc_getClass("SBWorkspace") sharedInstance] ZY_closeCurrentView];
	[[objc_getClass("SBMainWorkspace") sharedInstance] ZY_showWidgetSelector];
}
...
- (void)showAppWithSliderProvider:(__weak ZYAppSliderProviderView*)view {
	...
	[view load];
	[[objc_getClass("SBMainWorkspace") sharedInstance] ZY_setView:view preferredHeight:view.frame.size.height];
}
